...
PP_Bool Instance_DidCreate(PP_Instance instance_id, uint32_t argc, const char* argn[], const char* argv[]) 
{
  ...
  PPBInputEvent->RequestInputEvents(instance_id, PP_INPUTEVENT_CLASS_MOUSE | PP_INPUTEVENT_CLASS_KEYBOARD);
...
void Instance_DidChangeView(PP_Instance instance_id, PP_Resource view_resource)
{
  ...
  if ( !PPBView->GetRect( view_resource, &position ) )
  {
    return;
  }

  if ( !PPBView->GetClipRect( view_resource, &clip ) )
  {
    return;
  }
  ...
  if (global.graphics3d_id == kInvalidResource) 
  {
    ...
    global.graphics3d_id = PPBGraphics3D->Create(instance_id, kInvalidResource, attribs);
    int32_t success      = PPBInstance->BindGraphics(instance_id, global.graphics3d_id);
  ...
  {
    PPBGraphics3D->ResizeBuffers(global.graphics3d_id, position.size.width, position.size.height);
  }
...
PP_EXPORT int32_t PPP_InitializeModule(PP_Module module, PPB_GetInterface get_browser_interface) 
{
  ...
  PPBAudio           = (const PPB_Audio*) get_browser_interface(PPB_AUDIO_INTERFACE);
  PPBAudioConfig     = (const PPB_AudioConfig*) get_browser_interface(PPB_AUDIO_CONFIG_INTERFACE);
  ...
  PPBOpenGLES2       = (const PPB_OpenGLES2*) get_browser_interface(PPB_OPENGLES2_INTERFACE);
  PPBInputEvent      = (const PPB_InputEvent*) get_browser_interface(PPB_INPUT_EVENT_INTERFACE);
  PPBGraphics3D      = (const PPB_Graphics3D*) get_browser_interface(PPB_GRAPHICS_3D_INTERFACE);
  PPBMouseInputEvent = (const PPB_MouseInputEvent*) get_browser_interface(PPB_MOUSE_INPUT_EVENT_INTERFACE);
  PPBView            = (const PPB_View*) get_browser_interface(PPB_VIEW_INTERFACE);

  if (!PPBAudio || !PPBAudioConfig || !PPBCore || !PPBInputEvent || !PPBInstance || !PPBOpenGLES2 || !PPBGraphics3D || !PPBMouseInputEvent || !PPBView )
  {
    return -1;
  }
